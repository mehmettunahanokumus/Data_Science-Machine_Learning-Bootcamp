...
['NUM_' + col.upper() if df[col].dtypes in [int, float] else col.upper() for col in df.columns if 'NUM_' not in df.columns]
...
[col.upper() + '_FLAG' if 'no' not in df.columns else col.upper() for col in df.columns ]

# ###############################################
# # Görev 3: List Comprehension yapısı kullanarak aşağıda verilen değişken
# isimlerinden FARKLI olan değişkenlerin isimlerini seçiniz ve yeni bir dataframe oluşturunuz.
# ###############################################
